...
// ===========================================================================
// OPTION A — paste these two functions directly into server1.js,
// right AFTER resolveOrderSam (around line 1929).
//
// They are `function` DECLARATIONS, so they hoist — placement anywhere at
// module top-level is fine. Do NOT add a module.exports line; everything is
// used inside server1.js.
// ===========================================================================
    ...
    if (want.num != null) {
      const family = rows
        ...
        .sort((a, b) => {
          ...
          if (d !== 0) return d;
          return a.s.num - b.s.num;   // tie -> deterministic (lower version)
        });
      if (family.length) {
...
/* ===========================================================================
   CALL-SITE 1 — assignment / draft-run creation (server1.js ~line 6307).
   Replace the `tpl` SELECT + the `if (!src) { planner_lines }` fallback block
   (lines ~6307-6324) with:
   ...
   Keep the operators/hours/eff/sam/target math that follows EXACTLY as-is.
   resolveOrderSam still runs first, so the real merchant SAM wins and only the
   neighbour's operators/efficiency are borrowed:
     ...
     // targetPcs / targetPerHour unchanged
   ===========================================================================


   CALL-SITE 2 — operators-modal save (server1.js ~line 2015), the `tmpl` query.
...
// ===========================================================================
// OPTION A — RECOMPUTE LAYER for line_runs. Paste the THREE functions below
// directly into server1.js, right after resolveTemplateRun (~line 1929), so
// they sit next to resolveOrderSam. They are `async function` DECLARATIONS,
// so they hoist — recomputeStyleSafely can be referenced up in the
// registerWorkOrders(...) deps at line ~890 even though it's defined here.
// Do NOT add a module.exports line; everything is used inside server1.js.
//
// PREREQUISITES (do these first):
//   • the uq_line_run UNIQUE constraint must exist on the live line_runs table
//   • run the migration in section (0) below
//   • resolveOrderSam + resolveTemplateRun must already be present
// ===========================================================================


// ---------------------------------------------------------------------------
// (0) MIGRATION — paste these two lines into your initSchema block, right
//     AFTER the existing is_draft ALTER (server1.js line 359). ADD COLUMN IF
//     NOT EXISTS is idempotent, safe to re-run on every boot.
//
//       await client.query(`ALTER TABLE line_runs ADD COLUMN IF NOT EXISTS param_source   TEXT;`);
//       await client.query(`ALTER TABLE line_runs ADD COLUMN IF NOT EXISTS based_on_style TEXT;`);
//
//     param_source values: 'exact' | 'nearest' | 'line-recent' | 'planner' |
//     'default' | 'manual'. 'manual' = a planner confirmed the operator count,
//     so recompute must never reseed operators/efficiency over it.
// ---------------------------------------------------------------------------


/**
 * Refresh a single run's SAM (merchant-first) and recompute its target.
 * If the run is still a DRAFT whose params were never confirmed by a planner
 * (param_source is an estimate), also reseed operators/hours/efficiency from
 * the best template now available. A 'manual' run keeps its operator count.
 *
 * Returns { updated, runId?, source?, basedOnStyle?, sam?, operators?, targetPcs?, reason? }.
 */
async function recomputeRun(client, runId, { reseedEstimates = true } = {}) {
  const { rows } = await client.query(
    `SELECT id, line_no, to_char(run_date,'YYYY-MM-DD') AS run_date, style,
            operators_count, working_hours, sam_minutes, efficiency,
            work_order_id, is_draft, COALESCE(param_source,'') AS param_source
       FROM line_runs
      WHERE id = $1`,
    [runId]
  );
  if (!rows[0]) return { updated: false, reason: "not-found" };
  const run = rows[0];

  let operators = parseInt(run.operators_count) || 0;
  let hours     = parseFloat(run.working_hours) || 0;
  let eff       = parseFloat(run.efficiency) || 0;
  let source    = run.param_source || null;
  let basedOn   = null;

  // Only estimated + still-draft runs may have their roster reseeded.
  const isEstimate = ["nearest", "line-recent", "default", ""].includes(run.param_source);
  if (reseedEstimates && run.is_draft && isEstimate) {
    const tpl = await resolveTemplateRun(client, { line: run.line_no, style: run.style });
    if (tpl.templateRunId !== run.id) {           // never reseed from itself
      operators = parseInt(tpl.src?.operators_count) || operators;
      hours     = parseFloat(tpl.src?.working_hours) || hours;
      eff       = parseFloat(tpl.src?.efficiency)    || eff;
      source    = tpl.source;
      basedOn   = tpl.basedOnStyle;
    }
  }
  if (eff > 1) eff = eff / 100;   // tolerate 85 meaning 0.85
  if (eff > 1) eff = 1;

  // SAM: real merchant SAM first, fall back to whatever the run already had.
  const orderSam = await resolveOrderSam(client, {
    workOrderId: run.work_order_id,
    lineNo:      run.line_no,
    runDate:     run.run_date,
    style:       run.style,
  });
  const sam = orderSam || parseFloat(run.sam_minutes) || 0;

  if (!(sam > 0) || !(hours > 0) || operators <= 0) {
    return { updated: false, reason: "insufficient-inputs", runId: run.id };
  }
  ...
  await client.query(
    `UPDATE line_runs
        SET operators_count = $1,
            ...
            target_pcs      = $5,
            target_per_hour = $6,
            param_source    = COALESCE($7, param_source),
            based_on_style  = $8,
            updated_at      = now()
      WHERE id = $9`,
    [operators, hours, eff, sam, targetPcs, targetPerHour, source, basedOn, run.id]
  );

  return { updated: true, runId: run.id, source, basedOnStyle: basedOn, sam, operators, targetPcs, targetPerHour };
}


/**
 * Recompute every run of a given style (optionally scoped to one line).
 * Case-insensitive match on line_runs.style. Returns a small summary.
 */
async function recomputeRunsForStyle(client, { style, line = null }) {
  if (!style || String(style).trim() === "") return { matched: 0, updated: 0 };
  const params = [String(style).trim().toUpperCase()];
  let where = `UPPER(TRIM(style)) = $1`;
  if (line != null) { params.push(String(line)); where += ` AND line_no = $${params.length}`; }

  const { rows } = await client.query(
    `SELECT id FROM line_runs WHERE ${where} ORDER BY run_date`,
    params
  );

  let updated = 0;
  for (const r of rows) {
    const res = await recomputeRun(client, r.id);
...
/**
 * Self-contained wrapper for hooks in OTHER modules (work-orders.js). Manages
 * its own client + schema so it never touches a caller's transaction, and
 * swallows errors (a recompute hiccup must not fail the user's save). Relies on
 * `pool` and `setSchema`, both defined early in server1.js.
 */
async function recomputeStyleSafely({ style, line = null }) {
  const c = await pool.connect();
  try {
    await setSchema(c);
    return await recomputeRunsForStyle(c, { style, line });
  } catch (e) {
    console.warn("⚠️  recompute after SAM change failed:", e.message);
  } finally {
    c.release();
  }
}
